...
def requires_complete(method):
    @wraps(method)
    def wrapper(self, *args, **kwargs):
        if getattr(self, "status", None) != StatusType.COMPLETE:
            raise RuntimeError(
                f"Method '{method.__name__}' requires inference to be complete. "
                f"Current status = {getattr(self, 'status', None)!r}"
            )
...
class DataType(Enum):
    """ Types of supported data values. """
...
class Bound:
    ...
    def encompasses(self, v, inclusive: bool = True) -> bool:
        ...
        If `v` is dimensionless, assumes that `v` has units equal to that of
        `lower` and `upper`.
...
class IntegratedFlux(FluxBase, Integrable):
    ...
    def to_spectral(self, unit: str | u.Unit = u.mJy) -> SpectralFlux:
        ...
        unit : str or astropy.units.Unit, optional, default=u.mJy
            The unit to return the spectral flux density. Must be a spectral
            flux density unit.
...
class SpectralIndex(Integrable):
    ...
    def __init__(
        ...
        int_lower: u.Quantity,
        int_upper: u.Quantity,
        time_lower: u.Quantity = None,
        time_upper: u.Quantity = None
    ):
    ...
    def from_csv_row(cls, row):
        ...
        ------
        IOError
            Catches any error within the method and redirects into an IOError
        ...
        try:
            ...
            if hasattr(row, 'TimeLower') and not np.isnan(row.TimeLower):
                params['time_lower'] = u.Quantity(
                    row.TimeLower, row.TimeUnits
                ).to('d')

            if hasattr(row, 'TimeUpper') and not np.isnan(row.TimeUpper):
                params['time_upper'] = u.Quantity(
                    row.TimeUpper, row.TimeUnits
                ).to('d')
